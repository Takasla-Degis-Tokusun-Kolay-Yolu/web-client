...
export const getSpecificUserOffers = (id) => async (dispatch) => {
    try {
        dispatch({ type: START_OFFER_LOADING });
        const { data } = await api.fetchSpecificUserIncomingOffers(id);
        dispatch({ type: FETCH_SPECIFIC_USER_INCOMING_OFFERS, payload: data });
        dispatch({ type: END_OFFER_LOADING });
    } catch (error) {
        message.error(error.message);
    }
